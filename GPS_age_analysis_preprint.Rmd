...
```{r descriptives, eval = FALSE, echo=FALSE, include=FALSE}

sum((df$gender) == "Male")
sum((df$gender) == "Female")

#summary descriptives
v <- df %>% select(c("subj_math_skills.r", "age.r", "financial_wellbeing.r", "physical_wellbeing.r", "trust",
                     "altruism", "posrecip", "negrecip", "WP108", "WP109", "WP110"))
describe(v, skew = FALSE, quant = 0.5) %>% kbl(digits = 2, format = "html") %>% save_kable("descriptives.html")
...
scatter_plot(df$age.r, df$posrecip)
scatter_plot(df$age.r, df$altruism)
scatter_plot(df$age.r, df$negrecip) + ylab("Negative Reciprocity")

#Distributions of control variables
df %>% ggplot(aes(x = financial_wellbeing)) + geom_histogram(colour="black", fill="grey", binwidth = 0.1) + xlab("Age") + ylab("Count") + theme_apa() 
df %>% ggplot(aes(x = physical_wellbeing)) + geom_histogram(colour="black", fill="grey", binwidth = 0.1) + xlab("Age") + ylab("Count") + theme_apa() 
df %>% ggplot(aes(x = subj_math_skills)) + geom_histogram(colour="black", fill="grey", binwidth = 0.1) + xlab("Age") + ylab("Count") + theme_apa() 
...
ggplot(df,aes(x=age.r,y=prosocial_contrib,col=gender)) + geom_smooth(alpha=0.2, method = lm) + facet_wrap(~gender) + theme_apa() + xlab("Age") + ylab("Prosociality") + geom_point(alpha = 0.1)
...
# Run PREFERENCES LINEAR model with INTERACTION TERM
...
# Simpler version without all random effects
...
# Run model of BEHAVIOURS with interaction term = model.quad.behaviours

# model.quad.behav <- glmer (prosociality ~ gender + financial_wellbeing + physical_wellbeing + subj_math_skills + poly(age,2)*behav +
#                      (1 | id:country) + (1 | country) + #(0 + gender | country) +
#                      (0 + financial_wellbeing | country) + (0 + physical_wellbeing | country) +
#                      (0 + subj_math_skills | country) + (0 + age | country), 
#                      data = df.behav, 
#                      family = "binomial", 
...
save(model.quad.behav, file = paste0("output/Behaviours_linear_model.RData"))
...
for (c in country.results$isocode) {
  ...
  #calculate basic mean for each DV and add to country.results
  ...
  # country.results[country.results$isocode == c, 3:7] = c(tail(cntry.p$Coefficient,1),
  ...
  # country.results.p[country.results.p$isocode == c, 3:7] = c(tail(cntry.p$p,1),
  #                                                         tail(cntry.n$p,1), 
  #                                                         tail(cntry.a$p,1), 
  #                                                         tail(cntry.t$p,1),
  #                                                         tail(cntry.pr$p,1))
...
global_pref_map <- function(v, name) {
  ...
  draw_plot_label(name, x = -180, y = -30, hjust = 0, vjust = 0, angle = 90, size = 15, fontface = "plain")
  
}

#model outputs DV~Age
global_pref_map(df.world$posrecip.age, "Positive Reciprocity ~ Age")
global_pref_map(df.world$negrecip.age, "Negative Reciprocity ~ Age")
global_pref_map(df.world$altruism.age, "Altruism ~ Age")
global_pref_map(df.world$posrecip.age, "Trust ~ Age")
...
global_pref_map(df.world$negrecip.mean, "Negative Reciprocity")
global_pref_map(df.world$altruism.mean, "Altruism")
global_pref_map(df.world$posrecip.mean, "Trust")
