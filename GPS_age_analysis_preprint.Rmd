...
## PCA: note must be run as part od data set up
...
p5 <- plot_coeff(df.mod, linear.term5, quadratic.term5, pref.labs[5], total.std.error5) 
...
#for (v in 1:1) {
  ...
  assign(paste0("linear.term", v+5), as.numeric(as_tibble(model_parameters(model.quad, effects = "fixed")) %>% filter(Parameter == "poly(age, 2)1") %>% select(Coefficient)))
  assign(paste0("quadratic.term", v+5), as.numeric(as_tibble(model_parameters(model.quad, effects = "fixed")) %>% filter(Parameter == "poly(age, 2)2") %>% select(Coefficient)))
 assign(paste0("lin.std.error",v), as.numeric(as_tibble(model_parameters(model.quad, effects = "fixed")) %>% filter(Parameter == "poly(age, 2)1") %>% select(SE)))
  assign(paste0("quad.std.error",v), as.numeric(as_tibble(model_parameters(model.quad, effects = "fixed")) %>% filter(Parameter == "poly(age, 2)2") %>% select(SE)))
  ...
  assign(paste0("total.std.error",v+5), as.numeric(sqrt(t(a) %*% vcov(model.quad) %*% a)) ) #calculate std error of linear and quadratic terms combined and assign to total.std.error
...
p6 <- plot_coeff(df.mod, linear.term6, quadratic.term6, behav.labs[1], total.std.error6) 
p7 <- plot_coeff(df.mod, linear.term7, quadratic.term7, behav.labs[2], total.std.error7) 
p8 <- plot_coeff(df.mod, linear.term8, quadratic.term8, behav.labs[3], total.std.error8) 
...
plot_quart <- function(d, lin, quad, y_lab, w) {
  ggplot(d, aes(x = age.r, y = (lin * age) + (quad * (age^2) ) )) +
  geom_smooth() +
...
for (v in 1:length(prefs)) { #loop of each preference
  ...
  p <- plot_quart(df.mod, fixef(model.quad)["poly(age, 2)1"], fixef(model.quad)["poly(age, 2)2"], pref.lab, w ) # note: last term calculates pref.labs index
  print(p)
...
grid.arrange(qplotProsociality1, qplotProsociality2, qplotProsociality3, `qplot Prosociality4`, nrow = 1)
...
for (v in 1:length(behavs)) { #loop of each behaviour
  ...
  p <- plot_quart(df.mod, fixef(model.quad)["poly(age, 2)1"], fixef(model.quad)["poly(age, 2)2"], behav.lab, w ) # note: last term calculates behav.labs index - NEED TO FIX!
  print(p)
...
## GNI quartile model fits and plots  - NEW CODE
