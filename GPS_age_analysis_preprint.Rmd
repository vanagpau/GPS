...
for (c in country.results$isocode) {
  ...
  cntry.d <- model_parameters(glm(WP108 ~ gender + age, family = binomial, data = cdf, standardize = "refit"))
  cntry.v <- model_parameters(glm(WP109 ~ gender + age,  family = binomial, data = cdf, standardize = "refit"))
  cntry.h <- model_parameters(glm(WP110 ~ gender + age,  family = binomial, data = cdf, standardize = "refit"))
...
```{r economic-social-moderation, eval = FALSE}
...
for (v in 1:length(prefs)) {
  ...
  #load(model.quad, file = paste0("output/",pref.lab, "_GNI_model.RData"))
