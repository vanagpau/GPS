...
plot_list <- vector('list', length(prefs)) #list for storing plots
...
p2 <- plot_coeff(linear.term2, quadratic.term2, pref.labs[2]) 
...
#for (v in 1:1) {
  ...
  #first extract terms
  linear.term <- as_tibble(model_parameters(model.quad, effects = "fixed")) %>% filter(Parameter == "poly(age, 2)1") %>% select(Coefficient)
  linear.term <- as.numeric(linear.term)
  quadratic.term <- as_tibble(model_parameters(model.quad, effects = "fixed")) %>% filter(Parameter == "poly(age, 2)2") %>% select(Coefficient)
  quadratic.term <- as.numeric(quadratic.term)
  
  
  #create plot from model coefficients 
  plot.coeff <- ggplot(df.mod, aes(x = age.r, y = (linear.term * age) + (quadratic.term * (age^2) ) )) +
     geom_smooth() +
     scale_x_continuous(name = "Age") + 
    theme_classic() + 
    theme(axis.text = element_markdown(size = axtext),
