...
#create quartile rank variable for family ties
df$family.quart <- ntile(df$family, 4)
...
```{r descriptives, eval = FALSE, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r pref-models-plots, eval = FALSE, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
...
plot_coeff <- function(lin, quad, y_lab) {
 ggplot(df.mod, aes(x = age.r, y = (lin * age) + (quad * (age^2) ) )) +
   geom_smooth() +
   scale_x_continuous(name = "Age") +
  theme_apa() +
  theme(axis.text = element_markdown(size = axtext),
...
p1 <- plot_coeff(linear.term1, quadratic.term1, pref.labs[1])  
p2 <- plot_coeff(linear.term2, quadratic.term2, pref.labs[2]) # Negative Reciprocity
p3 <- plot_coeff(linear.term3, quadratic.term3, pref.labs[3]) 
p4 <- plot_coeff(linear.term4, quadratic.term4, pref.labs[4]) 
p5 <- plot_coeff(linear.term5, quadratic.term5, pref.labs[5]) 
...
p6 <- plot_coeff(linear.term6, quadratic.term6, behav.labs[1]) 
p7 <- plot_coeff(linear.term7, quadratic.term7, behav.labs[2]) 
p8 <- plot_coeff(linear.term8, quadratic.term8, behav.labs[3]) 
...
```{r country-effects, echo=FALSE, eval = FALSE, warning=FALSE, fig.align="center", fig.width=10, fig.height=8}
...
```{r country-plots, echo=FALSE, eval = FALSE, fig.align="center", fig.width=10, fig.height=8}
...
```{r GNI-moderation, eval = FALSE, echo = FALSE, warning = FALSE}
...
```{r Family-moderation, eval = FALSE, echo = FALSE, warning = FALSE}
