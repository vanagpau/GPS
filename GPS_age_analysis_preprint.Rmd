...
## TODAY'S JOB.. FIX THIS.. do the linear and quad line separately ..
...
plot_linquad <- function(d, lin, quad, y_lab, SE_lin, SE_quad) {
  ...
  ggplot(d) + geom_smooth(aes(x = age.r, y = yquad)) +
  geom_smooth(aes(x = age.r, y = ylin )) +
    geom_ribbon(aes(x = age.r, ymin = CIquad_lower, ymax = CIquad_upper), alpha=0.1, fill = "green", 
              color = "black", linetype = "dotted") +
    geom_ribbon(aes(x = age.r, ymin = CIlin_lower, ymax = CIlin_upper), alpha=0.1, fill = "green", 
              color = "black", linetype = "dotted") +
   scale_x_continuous(name = "Age", breaks = c(seq(15, 75, 10)) ) +
  theme_apa() +
  theme(axis.text = element_markdown(size = axtext),
...
plot_linquad(df.mod, linear.term1, quadratic.term1, pref.labs[1], lin.std.error1, quad.std.error1)  
...
#use previous plot_coeff function

p6 <- plot_coeff(df.mod, linear.term6, quadratic.term6, behav.labs[1], total.std.error6) + ylim(-150, 120)
p7 <- plot_coeff(df.mod, linear.term7, quadratic.term7, behav.labs[2], total.std.error7) + ylim(-150, 120)
p8 <- plot_coeff(df.mod, linear.term8, quadratic.term8, behav.labs[3], total.std.error8) + ylim(-150, 120)
...
for (v in 1:length(prefs)) { #loop of each preference
  ...
  p <- plot_quart(df.mod, fixef(model.quad)["poly(age, 2)1"], fixef(model.quad)["poly(age, 2)2"], pref.lab, w, as.numeric(sqrt(t(a) %*% vcov(model.quad) %*% a)) ) # note: last term calculates Std Error to pas to plot function
  print(p)
