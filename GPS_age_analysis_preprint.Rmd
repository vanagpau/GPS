...
plot_coeff <- function(d, lin, quad, y_lab) {
...
p1 <- plot_coeff(df.mod, linear.term1, quadratic.term1, pref.labs[1])  
p2 <- plot_coeff(df.mod, linear.term2, quadratic.term2, pref.labs[2]) # Negative Reciprocity
p3 <- plot_coeff(df.mod, linear.term3, quadratic.term3, pref.labs[3]) 
p4 <- plot_coeff(df.mod, linear.term4, quadratic.term4, pref.labs[4]) 
p5 <- plot_coeff(df.mod, linear.term5, quadratic.term5, pref.labs[5]) 
...
p6 <- plot_coeff(df.mod, linear.term6, quadratic.term6, behav.labs[1]) 
p7 <- plot_coeff(df.mod, linear.term7, quadratic.term7, behav.labs[2]) 
p8 <- plot_coeff(df.mod, linear.term8, quadratic.term8, behav.labs[3]) 
...
i <- 1 # counter variable for naming plots/models
...
for (v in 1:length(prefs)) { #loop of each preference
  ...
  p <- plot_quart(df.mod, fixef(model.quad)["poly(age, 2)1"], fixef(model.quad)["poly(age, 2)2"], pref.labs[ (((i-1)%/%4)+1) ], w ) # note: last term calculates pref.labs index
  print(p)
  assign(paste0("plot",i), p, envir = .GlobalEnv)
...
# like this
# for (i in 1:length(mod_pref_quart)) {
#   print(((i-1)%/%4)+1)
#   print(fixef(mod_pref_quart[[i]])["poly(age, 2)1"])
#   print(fixef(mod_pref_quart[[i]])["poly(age, 2)2"])
# }


grid.arrange(plot1, plot2, plot3, plot4, nrow = 1)
grid.arrange(plot5, plot6, plot7, plot8, nrow = 1)
grid.arrange(plot9, plot10, plot11, plot12, nrow = 1)
grid.arrange(plot13, plot14, plot15, plot16, nrow = 1)
grid.arrange(plot17, plot18, plot19, plot20, nrow = 1)
...
for (v in 1:length(behavs)) { #loop of each behaviour
  ...
  p <- plot_quart(df.mod, fixef(model.quad)["poly(age, 2)1"], fixef(model.quad)["poly(age, 2)2"], behav.labs[ (((i-1)%/%4)+1) ], w ) # note: last term calculates behav.labs index
  print(p)
  assign(paste0("plot",i), p, envir = .GlobalEnv)
...
# like this
# for (i in 1:length(mod_behav_quart)) {
#   print(((i-1)%/%4)+1)
#   print(fixef(mod_pref_quart[[i]])["poly(age, 2)1"])
#   print(fixef(mod_pref_quart[[i]])["poly(age, 2)2"])
# }
...
grid.arrange(plot21, plot22, plot23, plot24, nrow = 1)
