...
plot_quart <- function(d, lin, quad, y_lab) {
  ggplot(d, aes(x = age.r, y = (lin * age) + (quad * (age^2) ) )) +
  geom_smooth() +
  scale_x_continuous(name = "Age") +
  theme_apa() +
...
for (v in 1:length(prefs)) { #loop of each preference
  ...
  p <- plot_quart(df.mod, fixef(model.quad)["poly(age, 2)1"], fixef(model.quad)["poly(age, 2)2"], pref.labs[ (((i-1)%/%4)+1) ] ) # note: last term calculates pref.labs index
  print(p)
