...
df %>% ggplot(aes(x = posrecip)) + geom_histogram(colour="black", fill="grey", binwidth = 0.1) + xlab("Age") + ylab("Count") + theme_apa() 

# Histogram of age distribution Negative Reciprocity
df %>% ggplot(aes(x = negrecip)) + geom_histogram(colour="black", fill="grey", binwidth = 0.1) + xlab("Age") + ylab("Count") + theme_apa() 

# Histogram of age distribution Altruism
df %>% ggplot(aes(x = altruism)) + geom_histogram(colour="black", fill="grey", binwidth = 0.1) + xlab("Age") + ylab("Count") + theme_apa() 

# Histogram of age distribution Trust
df %>% ggplot(aes(x = trust)) + geom_histogram(colour="black", fill="grey", binwidth = 0.1) + xlab("Age") + ylab("Count") + theme_apa() 
...
scatter_plot <- function(y) {
  df %>% ggplot(aes(x = age.r, y = y, group = country)) + 
    geom_point(
        color="#69b3a2",
        fill="#69b3a2",
        shape=21,
...
scatter_plot(df$prosocial_contrib) + ylab("Prosociality")
scatter_plot(df$posrecip)
scatter_plot(df$altruism)
scatter_plot(df$negrecip) + ylab("Negative Reciprocity")
...
# Run PREFERENCES QUADRATIC model
...
for (v in 1:1) {
  ...
  # model.quad[v] <- glmer(outcome ~ gender + financial_wellbeing + physical_wellbeing + subj_math_skills + poly(age,2) +
  #                      (1 | country) + #(0 + gender | country) +
  #                      (0 + financial_wellbeing | country) + (0 + physical_wellbeing | country) + (0 + subj_math_skills | country) +
  #                      (0 + age | country), data = df.mod, family = "binomial", control = glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
  
    model.quad[v] <- glmer(outcome ~ gender + financial_wellbeing + physical_wellbeing + subj_math_skills + poly(age,2) + (1 | country), data = df.mod, family = "binomial", control = glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
  
  # save the model
  save(model.quad[v], file = paste0("output/",behav.lab, "_quadratic_model.RData"))
