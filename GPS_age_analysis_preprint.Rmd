...
c1 <- df %>% select(age, financial_wellbeing, physical_wellbeing, subj_math_skills) %>% cor(use = "complete.obs")
colnames(c1) <- c("Age", "Financial well-being", "Physical well-being", "Subjective Maths skills")
rownames(c1) <- c("Age", "Financial well-being", "Physical well-being", "Subjective Maths skills")
...
p.mat <- cor.mtest(df %>% select(age, financial_wellbeing, physical_wellbeing, subj_math_skills))
colnames(p.mat) <- c("Age", "Financial well-being", "Physical well-being", "Subjective Maths skills")
rownames(p.mat) <- c("Age", "Financial well-being", "Physical well-being", "Subjective Maths skills")


corrplot::corrplot(c1, method = "circle", order = "FPC", col = colorRampPalette(c("blue", "white", "red"))(20), p.mat = p.mat, sig.level = 0.05, addCoef.col = "black", tl.col="black", tl.srt=45, tl.cex = .75, insig = "blank", number.font = 1, diag = FALSE)
