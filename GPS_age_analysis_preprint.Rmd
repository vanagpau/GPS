...
# Add country level 'Family' variable to data-frame

# Load previously saved Dataframe from WVS_analysis.Rmd
load("family.Rda")
#Join to dataframe
df <- left_join(df, fam_df, by = "isocode")
#Standardise
df$family <- scale(df$family)
...
#create quartile rank variable for GNI
df$gni.quart <- ntile(df$gni, 4)
...
## PCA: note must be run as part of data set up
...
result <- PCAtest(df.dropna, 100, 100, 0.05, varcorr=TRUE, counter=TRUE, plot=TRUE)
...
scatter_plot(df$age.r, df$prosocial_contrib) + ylab("Prosociality") + xlab("Age")

s1 <- scatter_plot(df$age.r, df$posrecip) + ylab("Positive Reciprocity") + xlab("Age")
s2 <- scatter_plot(df$age.r, df$altruism) + ylab("Altruism") + xlab("Age")
s3 <- scatter_plot(df$age.r, df$negrecip) + ylab("Negative Reciprocity") + xlab("Age")
s4 <- scatter_plot(df$age.r, df$trust) + ylab("Trust") + xlab("Age")
...
c1 <- df %>% select(age, financial_wellbeing, physical_wellbeing, subj_math_skills, posrecip, negrecip, altruism, trust, WP108, WP109, WP110, gender_index, prosocial_contrib) %>% cor(use = "complete.obs")
...
p.mat <- cor.mtest(df %>% select(age, financial_wellbeing, physical_wellbeing, subj_math_skills, posrecip, negrecip, altruism, trust, WP108, WP109, WP110, gender_index, prosocial_contrib))
...
## Overall preference and behaviour models - for differences
...
# create preference levels as factor for comparisons
df.pref$pref <- factor(df.pref$pref)

for (v in 1:length(unique(df.pref$pref))) {

# relevel of factors to generate A-B-C comparison

df.pref$pref <- relevel(df.pref$pref, ref = paste0(df.pref$pref[v]))
...
# 
# save(model.quad.pref, file = paste0("output/Pref_quad_mod_refcat_",df.pref$pref[v],".RData"))
...
#            data = df.pref, control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
# 
# save(model.lin.pref, file = paste0("output/Pref_lin_mod_refcat_",df.pref$pref[v],".RData"))

#load pre-existing model objects for PREFERENCES

load(paste0("output/Pref_lin_mod_refcat_",df.pref$pref[v],".RData"))
load(paste0("output/Pref_quad_mod_refcat_",df.pref$pref[v],".RData"))
...
print("Quadratic mixed-effects model predicting prosocial preferences")
print(r.quad.pref)

t <- kable(r.quad.pref) %>%  kable_styling()
save_kable(t, file = paste0("table_","Pref_quad_mod_refcat_",df.pref$pref[v],".html"))
...
print("Linear mixed-effects model predicting prosocial preferences")
print(r.lin.pref)
...
# create behaviour levels as factor for comparisons
df.behav$behav <- factor(df.behav$behav)

for (v in 1:length(unique(df.behav$behav))) {

# relevel of factors to generate A-B-C comparison

df.behav$behav <- relevel(df.behav$behav, ref = paste0(df.behav$behav[v]))
...
# toc-tic
# 
# save(model.quad.behav, file = paste0("output/Behav_quad_mod_refcat_",df.behav$behav[v],".RData"))
...
# 
# save(model.lin.behav, file = paste0("output/Behav_lin_mod_refcat_",df.behav$behav[v],".RData"))


load(paste0("output/Behav_lin_mod_refcat_",df.behav$behav[v],".RData"))
load(paste0("output/Behav_quad_mod_refcat_",df.behav$behav[v],".RData"))


results.quad.behav <- model_parameters(model.quad.behav, exponentiate = FALSE, effects = "fixed")
...
print("Quadratic mixed-effects model predicting prosocial behaviours")
print(r.quad.behav)

t <- kable(r.quad.behav) %>%  kable_styling()
save_kable(t, file = paste0("table_","Behav_quad_mod_refcat_",df.behav$behav[v],".html"))
...
results.lin.behav <- model_parameters(model.lin.behav, exponentiate = FALSE, effects = "fixed")
...
print("Linear mixed-effects model predicting prosocial behaviours")
print(r.lin.behav)
...
for (v in 1:length(prefs)) {
  ...
  #save html table to file (note: print is required as sjPlot does not render in Rmd)

  #print(tab_model(model.quad, file = paste0(pref.lab, "_quadratic_model.html")))
  ...
  #plot the quadratic line from fixed effect coefficients in the model (rather than separate linear model as above)
  
  #first extract terms
  assign(paste0("linear.term",v), as.numeric(as_tibble(model_parameters(model.quad, effects = "fixed")) %>% filter(Parameter == "poly(age, 2)1") %>% select(Coefficient)))
  assign(paste0("quadratic.term",v), as.numeric(as_tibble(model_parameters(model.quad, effects = "fixed")) %>% filter(Parameter == "poly(age, 2)2") %>% select(Coefficient)))
  assign(paste0("lin.std.error",v), as.numeric(as_tibble(model_parameters(model.quad, effects = "fixed")) %>% filter(Parameter == "poly(age, 2)1") %>% select(SE)))
  assign(paste0("quad.std.error",v), as.numeric(as_tibble(model_parameters(model.quad, effects = "fixed")) %>% filter(Parameter == "poly(age, 2)2") %>% select(SE)))
  
  
  #Method 1 from here: https://stats.stackexchange.com/questions/446676/measuring-standard-error-of-two-or-more-coefficients-combined and here https://aosmith.rbind.io/2018/11/16/plot-fitted-lines/
 
  a <- c(0,0,0,0,0,1,1) # vector to pick out coefficients from which to calculate SE - this depends on order of effects from summary(model.quad)
  
  assign(paste0("total.std.error",v), as.numeric(sqrt(t(a) %*% vcov(model.quad) %*% a)) ) #calculate std error of linear and quadratic terms combined and assign to total.std.error
...
#create function to plot from model coefficients

plot_coeff <- function(d, lin, quad, y_lab, SE) {
  d$y <- (lin * d$age) + (quad * (d$age^2) )
  d$CI_lower <- d$y - (1.96*SE) # 95% CI is 1.96 x Std Error
  d$CI_upper <- d$y + (1.96*SE)
 ggplot(d, aes(x = age.r, y = y)) +
   geom_smooth() +
   scale_x_continuous(name = "Age", breaks = c(seq(15, 75, 10)) ) +
    geom_ribbon(aes(ymin = CI_lower, ymax = CI_upper), alpha=0.1, fill = "green", 
              color = "black", linetype = "dotted") +
  theme_apa() +
  theme(axis.text = element_markdown(size = axtext),
        axis.title.x = element_markdown(size = axtitle/2),
        axis.title.y = element_markdown(size = axtitle/2)) +
  ylim(-30, 50) + ylab(y_lab)
}

# MAX plot_coeff function
plot_coeff_max <- function(d, lin, quad, y_lab, SE) {
  d$y <- (lin * d$age) + (quad * (d$age^2) )
  d$CI_lower <- d$y - (1.96*SE) # 95% CI is 1.96 x Std Error
  d$CI_upper <- d$y + (1.96*SE)
  peak <- which.max(d$y)
 ggplot(d, aes(x = age.r, y = y)) +
   ...
   scale_x_continuous(name = "Age", breaks = c(seq(15, 75, 10)) ) +
    geom_ribbon(aes(ymin = CI_lower, ymax = CI_upper), alpha=0.1, fill = "green", 
              color = "black", linetype = "dotted") +
   geom_vline(xintercept = d$age.r[peak], linetype = "dotted") +
   annotate("text", x = d$age.r[peak] + 1, y = -15, label = paste0("Peak age = ", d$age.r[peak])) +
  theme_apa() +
  theme(axis.text = element_markdown(size = axtext),
        axis.title.x = element_markdown(size = axtitle/2),
        axis.title.y = element_markdown(size = axtitle/2)) +
   ylim(-150, 120) + ylab(y_lab)
}


# Plot linear and quadratic on same function


plot_linquad <- function(d, lin, quad, y_lab, SE_lin, SE_quad) {
  d$yquad <- (lin * d$age) + (quad * (d$age^2) )
  d$CIquad_lower <- d$yquad - (1.96*SE_quad) # 95% CI is 1.96 x Std Error
  d$CIquad_upper <- d$yquad + (1.96*SE_quad)
  
  d$ylin <- (lin * d$age)
  d$CIlin_lower <- d$ylin - (1.96*SE_lin) # 95% CI is 1.96 x Std Error
  d$CIlin_upper <- d$ylin + (1.96*SE_lin)
  
  ggplot(d) + 
    geom_smooth(aes(x = age.r, y = yquad), colour = "green4") +
    geom_smooth(aes(x = age.r, y = ylin)) +
    geom_ribbon(aes(x = age.r, ymin = CIquad_lower, ymax = CIquad_upper), alpha=0.1, fill = "green", 
              color = "black", linetype = "dotted") +
    geom_ribbon(aes(x = age.r, ymin = CIlin_lower, ymax = CIlin_upper), alpha=0.1, fill = "#66FFFF", 
              color = "black", linetype = "dotted") +
   scale_x_continuous(name = "Age", breaks = c(seq(15, 75, 10)) ) +
  theme_apa() +
  theme(axis.text = element_markdown(size = axtext),
        axis.title.x = element_markdown(size = axtitle/2),
        axis.title.y = element_markdown(size = axtitle/2)) +
  ylim(-75, 50) + ylab(y_lab)
  
}

plot_linquad_max <- function(d, lin, quad, y_lab, SE_lin, SE_quad) {
  d$yquad <- (lin * d$age) + (quad * (d$age^2) )
  d$CIquad_lower <- d$yquad - (1.96*SE_quad) # 95% CI is 1.96 x Std Error
  d$CIquad_upper <- d$yquad + (1.96*SE_quad)
  peak <- which.max(d$yquad)
  d$ylin <- (lin * d$age)
  d$CIlin_lower <- d$ylin - (1.96*SE_lin) # 95% CI is 1.96 x Std Error
  d$CIlin_upper <- d$ylin + (1.96*SE_lin)
...
p1 <- plot_linquad_max(df.mod, linear.term1, quadratic.term1, pref.labs[1], lin.std.error1, quad.std.error1)  + ylim(-30, 20)
p3 <- plot_coeff_max(df.mod, linear.term3, quadratic.term3, pref.labs[3], total.std.error3) + ylim(-30, 20)

##
  
#p1 <- plot_coeff(df.mod, linear.term1, quadratic.term1, pref.labs[1], total.std.error1)  
p2 <- plot_coeff(df.mod, linear.term2, quadratic.term2, pref.labs[2], total.std.error2) # Negative Reciprocity
#p3 <- plot_coeff(df.mod, linear.term3, quadratic.term3, pref.labs[3], total.std.error3) 
p4 <- plot_coeff(df.mod, linear.term4, quadratic.term4, pref.labs[4], total.std.error4) 
p5 <- plot_coeff(df.mod, linear.term5, quadratic.term5, pref.labs[5], total.std.error5) # Trust
...
#for (v in 1:1) {
  ...
  #save html summary table to file

  print(tab_model(model.quad, transform = NULL, file = paste0(behav.lab, "_quadratic_model.html")))
  ...
  #first extract terms (note these are labelled 6-8 hence v+5)
  nv <- v+5 #new 'v'
  
  assign(paste0("linear.term", nv), as.numeric(as_tibble(model_parameters(model.quad, effects = "fixed")) %>% filter(Parameter == "poly(age, 2)1") %>% select(Coefficient)))
  assign(paste0("quadratic.term", nv), as.numeric(as_tibble(model_parameters(model.quad, effects = "fixed")) %>% filter(Parameter == "poly(age, 2)2") %>% select(Coefficient)))
 assign(paste0("lin.std.error",nv), as.numeric(as_tibble(model_parameters(model.quad, effects = "fixed")) %>% filter(Parameter == "poly(age, 2)1") %>% select(SE)))
  assign(paste0("quad.std.error",nv), as.numeric(as_tibble(model_parameters(model.quad, effects = "fixed")) %>% filter(Parameter == "poly(age, 2)2") %>% select(SE)))
  ...
  assign(paste0("total.std.error",nv), as.numeric(sqrt(t(a) %*% vcov(model.quad) %*% a)) ) #calculate std error of linear and quadratic terms combined and assign to total.std.error
...
#Donating linear and quadratic plot
plot_linquad(df.mod, linear.term6, quadratic.term6, behav.labs[1], lin.std.error6, quad.std.error6)  + ylim(-150, 120)


p6 <- plot_linquad_max(df.mod, linear.term6, quadratic.term6, behav.labs[1], lin.std.error6, quad.std.error6)  + ylim(-150, 120)

#p6 <- plot_coeff_max(df.mod, linear.term6, quadratic.term6, behav.labs[1], total.std.error6) 
p7 <- plot_coeff_max(df.mod, linear.term7, quadratic.term7, behav.labs[2], total.std.error7)
p8 <- plot_coeff_max(df.mod, linear.term8, quadratic.term8, behav.labs[3], total.std.error8)

grid.arrange(p6, p7, p8, ncol = 2)

```

## MODEL COMPARISONS AKAIKE method

```{r model-comparisons, eval = FALSE, echo = FALSE}

#Preferences models

#Create NA free dataframe to enable comparisons
df.compare.pref <- df %>% select(prosocial_contrib, negrecip, posrecip, altruism, trust, gender, financial_wellbeing, physical_wellbeing, subj_math_skills, age, country) %>% na.omit()

# Re-run model fits on new data-frame
...
for (v in 1:length(prefs)) {
  ...
  # create dataframe for modelling/plotting cycling through outcome variables one by one
  df.mod <- df.compare.pref %>%
    dplyr::rename("outcome" = paste(pref)) %>%
    drop_na(c("outcome"))
  
  # Model fitting #'ed for speed - load from previous runs (below)
  
model.quad <- lmer(age ~ gender + financial_wellbeing + physical_wellbeing + subj_math_skills + poly(outcome,2) +
                       (1 | country) + #(0 + gender | country) +
                       (0 + financial_wellbeing | country) + (0 + physical_wellbeing | country) + (0 + subj_math_skills | country) +
                       (0 + outcome | country),
  data = df.mod, control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))

  assign(paste0("model.compare.",pref), model.quad)

  save(list = paste0("model.compare.",pref), file = paste0("output/",pref.lab, "_quadratic_comparison.RData"))
  #load(file = paste0("output/",pref.lab, "_quadratic_comparison.RData"))
 
}

#Compare models using AIC weighting

models <- list(model.compare.altruism, model.compare.negrecip, model.compare.posrecip, model.compare.trust)
model.names <- c("Altruism", "Negative Recip", "Positive Recip", "Trust")
aictab(cand.set = models, modnames = model.names, REML = FALSE) %>% kbl() %>%
  kable_classic(full_width = F, html_font = "Arial")

models <- list(model.compare.altruism, model.compare.posrecip, model.compare.trust)
model.names <- c("Altruism", "Positive Recip", "Trust")
aictab(cand.set = models, modnames = model.names, REML = FALSE) %>% 
  kbl() %>%
  kable_classic(full_width = F, html_font = "Arial")

models <- list(model.compare.altruism, model.compare.posrecip)
model.names <- c("Altruism", "Positive Recip")
aictab(cand.set = models, modnames = model.names, REML = FALSE) %>% 
  kbl() %>%
  kable_classic(full_width = F, html_font = "Arial")


#Repeat for behaviours
...
for (v in 1:length(behavs)) {
  ...
  # Model fitting
model.quad <- lmer(age ~ gender + financial_wellbeing + physical_wellbeing + subj_math_skills + outcome +
                       (1 | country) + #(0 + gender | country) +
                       (0 + financial_wellbeing | country) + (0 + physical_wellbeing | country) + (0 + subj_math_skills | country) +
                       (0 + outcome | country),
  data = df.mod, control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))

  assign(paste0("model.compare.",behav), model.quad)

  save(list = paste0("model.compare.",behav), file = paste0("output/",pref.lab, "_quadratic_comparison.RData"))
  #load(file = paste0("output/",behav.lab, "_quadratic_comparison.RData"))
 
}

models <- list(model.compare.WP108, model.compare.WP109, model.compare.WP110)
model.names <- c("Donated", "Volunteered", "Helped stranger")
aictab(cand.set = models, modnames = model.names, REML = FALSE) %>% 
  kbl() %>%
  kable_classic(full_width = F, html_font = "Arial")

models <- list(model.compare.WP108, model.compare.WP109)
model.names <- c("Donated", "Volunteered")
aictab(cand.set = models, modnames = model.names, REML = FALSE)%>% 
  kbl() %>%
  kable_classic(full_width = F, html_font = "Arial")

models <- list(model.compare.WP110, model.compare.WP109)
model.names <- c("Helped", "Volunteer")
aictab(cand.set = models, modnames = model.names, REML = FALSE) %>% 
  kbl() %>%
  kable_classic(full_width = F, html_font = "Arial")
...
```{r country-effects, echo=FALSE, eval = TRUE, warning=FALSE, fig.align="center", fig.width=10, fig.height=8}

# cntry.list <- read.csv("data/List_countries.csv", sep = ",", header = TRUE) # list of countries
cntry.list <- df %>%   select(c("isocode", "country"))
...
# loop over each country = PREFERENCES
for (c in country.results$isocode) {
  ...
  # standardise continuous variables for modelling
...
# create new df for behaviours to deal with NAs
# 
# country.results.behav <- country.results %>% na.omit
# 
# # loop over each country = BEHAVIOURS
for (c in country.results$isocode) {

  cdf <- df %>%  filter(isocode == c) %>% select(WP108, WP109, WP110, gender, age) %>% na.omit

  if (dim(cdf)[1] > 20) { # exclude countries that do not have more than 20 valid observations

  country.results$donated.mean[country.results.p$isocode == c] = c(mean(cdf$WP108, na.rm = TRUE))
  country.results$volunteered.mean[country.results.p$isocode == c] = c(mean(cdf$WP109, na.rm = TRUE))
  country.results$helped.mean[country.results.p$isocode == c] = c(mean(cdf$WP110, na.rm = TRUE))

  cntry.d <- model_parameters(glm(WP108 ~ gender + age, family = binomial, data = cdf))
  cntry.v <- model_parameters(glm(WP109 ~ gender + age,  family = binomial, data = cdf))
  cntry.h <- model_parameters(glm(WP110 ~ gender + age,  family = binomial, data = cdf))

  country.results$donated.age[country.results$isocode == c] = tail(cntry.d$Coefficient,1)
  country.results$volunteered.age[country.results$isocode == c] = tail(cntry.v$Coefficient,1)
  country.results$helped.age[country.results$isocode == c] = tail(cntry.h$Coefficient,1)

  country.results.p$donated.age[country.results$isocode == c] = tail(cntry.d$p,1)
  country.results.p$volunteered.age[country.results$isocode == c] = tail(cntry.v$p,1)
  country.results.p$helped.age[country.results$isocode == c] = tail(cntry.d$h,1)

  }
}

# Standardise outputs
country.results$donated.mean <- scale(country.results$donated.mean)
country.results$volunteered.mean <- scale(country.results$volunteered.mean)
country.results$helped.mean <- scale(country.results$helped.mean)
...
```{r country-plots, echo=FALSE, eval = TRUE, fig.align="center", fig.width=10, fig.height=8}

# create function for plotting v = variable, name = name of variable

global_map <- function(v, name) {
...
p1 <- global_map(df.world$posrecip.age, "Positive Reciprocity ~ Age")
p2 <- global_map(df.world$negrecip.age, "Negative Reciprocity ~ Age")
p3 <- global_map(df.world$altruism.age, "Altruism ~ Age")
p4 <- global_map(df.world$trust.age, "Trust ~ Age")
global_map(df.world$prosocial_contrib.age, "Prosociality ~ Age")

#grid arrange
grid.arrange(p1, p2, p3, p4)

#Basic means around the world
p1 <- global_map(df.world$posrecip.mean, "Positive Reciprocity")
p2 <- global_map(df.world$negrecip.mean, "Negative Reciprocity")
p3 <- global_map(df.world$altruism.mean, "Altruism")
p4 <- global_map(df.world$trust.mean, "Trust")
global_map(df.world$prosocial_contrib.mean, "Prosociality")
...
#Repeat for behaviours
p1 <- global_map(df.world$donated.mean, "Donated (mean)")
p2 <- global_map(df.world$volunteered.mean, "Volunteered (mean)")
p3 <- global_map(df.world$helped.mean, "Helped (mean)")

grid.arrange(p1, p2, p3, ncol = 2)

p1 <- global_map(df.world$donated.age, "Donated ~ Age")
p2 <- global_map(df.world$volunteered.age, "Volunteered ~ Age")
p3 <- global_map(df.world$helped.age, "Helped ~ Age")
...
```{r GNI-moderation, eval = TRUE, echo = FALSE, warning = FALSE}

## Re-run models with GNI data

# 1. Re-run PREFERENCE models with GNI
...
for (v in 1:length(prefs)) {
  ...
  # For speed of plotting cut out the model if required
  # Note: model does not converge if gender slope random effect is included
  
  # model.quad <- lmer(outcome ~ gender + financial_wellbeing + physical_wellbeing + subj_math_skills + poly(age,2) * gni +
  #                      (1 | country) +
  #                      # (0 + gender | country) +
  #                      (0 + financial_wellbeing | country) +
  #                      (0 + physical_wellbeing | country) +
  ...
  #                    data = df.mod, control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
  # 
  # save(model.quad, file = paste0("output/",pref.lab, "_GNI_model.RData"))
  load(file = paste0("output/",pref.lab, "_GNI_model.RData"))
  
  #save html table to file (note: print is required as sjPlot does not render in Rmd)

  print(tab_model(model.quad, file = paste0(pref.lab, "_GNI_model.html")))
  mod_pref[[v]] <- model.quad
  
  
  
}

tab_model(mod_pref[[1]], mod_pref[[2]], mod_pref[[3]], mod_pref[[4]], mod_pref[[5]])

  # 2. Re-run BEHAVIOUR models with GNI
...
for (v in 1:length(behavs)) {
  ...
  # model.quad <- glmer(outcome ~ gender + financial_wellbeing + physical_wellbeing + subj_math_skills + poly(age,2) * gni +
  #                     (1 | country) +
  #                     #(0 + gender | country) +
  #                     (0 + financial_wellbeing | country) +
  #                     (0 + physical_wellbeing | country) +
  #                     (0 + subj_math_skills | country) +
  #                     (0 + age | country),
  #                     nAGQ = 0, # speeds things up!
  #                     data = df.mod, family = "binomial", control = glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
  # 
  # 
  # 
  # # save the model
  # save(model.quad, file = paste0("output/",behav.lab, "_GNI_model.RData"))
  
  #load previously created model
  load(file = paste0("output/",behav.lab, "_GNI_model.RData"))
  mod_behav[[v]] <- model.quad

  #save html summary table to file

  print(tab_model(model.quad, transform = NULL, file = paste0(behav.lab, "_GNI_model.html")))
  
}

tab_model(mod_behav[[1]], mod_behav[[2]], mod_behav[[3]], transform = NULL)
...
## GNI quartile model fits and plots

```{r GNI-quartile-plots, eval = TRUE, echo = FALSE, warning = TRUE}

## 2. GNI QUARTILE - PREFERENCES


# initialise variables/lists
p <- list() # list of plots
mod_pref_quart <- list() # list of models
i <- 1 # counter variable for naming models

#define function for plotting quartiles

plot_quart <- function(d, lin, quad, y_lab, w, SE) {
  d$y <- (lin * d$age) + (quad * (d$age^2) )
  d$CI_lower <- d$y - (1.96*SE) # 95% CI is 1.96 x Std Error
  d$CI_upper <- d$y + (1.96*SE)
  ggplot(d, aes(x = age.r, y = y) ) +
  geom_smooth() +
  ggtitle(paste0(y_lab,"\nGNI Quartile ",w)) +
  scale_x_continuous(name = "Age", breaks = c(seq(15, 75, 10)) ) +
  geom_ribbon(aes(ymin = CI_lower, ymax = CI_upper), alpha=0.1, fill = "green", 
              color = "black", linetype = "dotted") +  
  theme_apa() +
...
for (v in 1:length(prefs)) { #loop of each preference
  
  pref <- prefs[v]
  pref.lab <- pref.labs[v]
  
  for (w in 1:4) { # loop for each quartile

  # create dataframe for modelling/plotting cycling through outcome variables one by one
  df.mod <- df %>% filter(gni.quart == w) %>% # filter by GNI quartile
    dplyr::rename("outcome" = paste(pref)) %>% 
    drop_na(c("outcome"))
  
  # NOTE: For speed of plotting cut out the model unless refit
  
  #Note: As these are quartiles, we do not model GNI within each quartile, so this term comes out of the regression equation
  ...
  #                    data = df.mod, control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
  # 
  # save(model.quad, file = paste0("output/",pref.lab, "_GNI_quartile",w,".RData"))

  load(file = paste0("output/",pref.lab, "_GNI_quartile",w,".RData"))
  
  #save html table to file (note: print is required as sjPlot does not render in Rmd)
  #print(tab_model(model.quad, file = paste0(pref.lab, "_GNI_quartile",w,".html")))

  mod_pref_quart[[i]] <- model.quad # First four models = preference 1, second four models pref 2 etc
  assign(paste0("mod_pref_quart",i), model.quad, envir = .GlobalEnv)
  
  #Test models for singularity issues
  print(paste0("mod_pref_quart",i))
  print(paste0("Model has singular fit issue = ", isSingular(mod_pref_quart[[i]])))
  
  #Create plots
  
  a <- c(0,0,0,0,0,1,1) # vector to pick out coefficients from which to calculate SE - this depends on order of effects from summary(model.quad)

  p <- plot_quart(df.mod, fixef(model.quad)["poly(age, 2)1"], fixef(model.quad)["poly(age, 2)2"], pref.lab, w, as.numeric(sqrt(t(a) %*% vcov(model.quad) %*% a)) )  + ylim (-30, 30)# note: as.numeric term calculates Std Error to pas to plot function
  print(p)
  assign(paste0("qplot",pref.lab,w), p, envir = .GlobalEnv)
  
  i <- i+1 #increment index counter for models/plots
  
  }
  
}

grid.arrange(qplotProsociality1, qplotProsociality2, qplotProsociality3, qplotProsociality4, nrow = 1)
grid.arrange(qplotTrust1, qplotTrust2, qplotTrust3, qplotTrust4, nrow = 1)
grid.arrange(qplotAltruism1, qplotAltruism2, qplotAltruism3, qplotAltruism4, nrow = 1)
grid.arrange(`qplotPositive reciprocity1`, `qplotPositive reciprocity2`, `qplotPositive reciprocity3`, `qplotPositive reciprocity4`, nrow = 1)
grid.arrange(`qplotNegative reciprocity1`, `qplotNegative reciprocity2`, `qplotNegative reciprocity3`, `qplotNegative reciprocity4`, nrow = 1)


## 2. GNI QUARTILE - BEHAVIOURS

# initialise variables/lists
mod_behav_quart <- list() # list of models


for (v in 1:length(behavs)) { #loop of each behaviour
  ...
  #                      (0 + subj_math_skills | country) +
  #                      (0 + age | country),
  #                     nAGQ = 0, #estimate for speed
  #                    data = df.mod, family = "binomial", 
  #                    control = glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
  # 
  # save(model.quad, file = paste0("output/",behav.lab, "_GNI_quartile",w,".RData"))

  load(file = paste0("output/",behav.lab, "_GNI_quartile",w,".RData"))
  
  # save html table to file (note: print is required as sjPlot does not render in Rmd)
  print(tab_model(model.quad, file = paste0(behav.lab, "_GNI_quartile",w,".html")))

  mod_behav_quart[[i]] <- model.quad # First four models = behaviour 1, second four models behaviour 2 etc
  assign(paste0("mod_behav_quart",i), model.quad, envir = .GlobalEnv)
  
  #Test models for singularity issues
  print(paste0("mod_behav_quart",i))
  print(paste0("Model has singular fit issue = ", isSingular(mod_behav_quart[[i]])))
  
  #Create plots
   a <- c(0,0,0,0,0,1,1) # vector to pick out coefficients from which to calculate SE - this depends on order of effects from summary(model.quad)
   
  p <- plot_quart(df.mod, fixef(model.quad)["poly(age, 2)1"], fixef(model.quad)["poly(age, 2)2"], behav.lab, w, as.numeric(sqrt(t(a) %*% vcov(model.quad) %*% a)) ) # note: last term calculates Std Error
  print(p)
  assign(paste0("qplot",behav.lab,w), p, envir = .GlobalEnv)
  
    i <- i+1 #increment index counter for models
...
grid.arrange(`qplotDonating money1`, `qplotDonating money2`, `qplotDonating money3`, `qplotDonating money4`, nrow = 1)
grid.arrange(qplotVolunteering1, qplotVolunteering2, qplotVolunteering3, qplotVolunteering4, nrow = 1)
grid.arrange(`qplotHelping a stranger1`, `qplotHelping a stranger2`, `qplotHelping a stranger3`, `qplotHelping a stranger4`, nrow = 1)
...
```{r Family-moderation, eval = TRUE, echo = FALSE, warning = FALSE}


# 1. Re-run PREFERENCE models with FAMILY
...
for (v in 1:length(prefs)) {
  ...
  model.quad <- lmer(outcome ~ gender + financial_wellbeing + physical_wellbeing + subj_math_skills + poly(age,2) * family +
                       (1 | country) +
                       # (0 + gender | country) +
                       (0 + financial_wellbeing | country) +
                       (0 + physical_wellbeing | country) +
  ...
  save(model.quad, file = paste0("output/",pref.lab, "_family_model.RData"))
  #load(file = paste0("output/",pref.lab, "_family_model.RData"))
  
  #save html table to file (note: print is required as sjPlot does not render in Rmd)

  print(tab_model(model.quad, file = paste0(pref.lab, "_family_model.html")))
  mod_pref[[v]] <- model.quad
...
tab_model(mod_pref[[1]], mod_pref[[2]], mod_pref[[3]], mod_pref[[4]], mod_pref[[5]])
 ...
 # 2. Re-run BEHAVIOUR models with FAMILY
...
for (v in 1:length(behavs)) {
  ...
  model.quad <- glmer(outcome ~ gender +
                      financial_wellbeing + physical_wellbeing + subj_math_skills + poly(age,2) * family +
                      (1 | country) +
                      #(0 + gender | country) + #removed to prevent singular fit
                      (0 + financial_wellbeing | country) + #removed to prevent singular fit
                      (0 + physical_wellbeing | country) +
                      (0 + subj_math_skills | country) +
                      (0 + age | country),
                      nAGQ = 0, # speeds things up!
  ...
  # save the model
  save(model.quad, file = paste0("output/",behav.lab, "_family_model.RData"))
  
  #load previously created model
  #load(file = paste0("output/",behav.lab, "_family_model.RData"))
  mod_behav[[v]] <- model.quad

  #save html summary table to file

  print(tab_model(model.quad, transform = NULL, file = paste0(behav.lab, "_family_model.html")))
