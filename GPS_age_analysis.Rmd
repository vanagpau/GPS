...
df %>% group_by(country) %>% count() %>% mutate(prop = 100 * n / total_obs)

total_obs <- sum(df.mod %>% group_by(country) %>% count() %>% pull(n))
df.mod %>% group_by(country) %>% count() %>% mutate(prop = 100 * n / total_obs)
