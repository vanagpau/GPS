...
df$age.quart <- gtools::quantcut(df$age.r, q = 4, na.rm = TRUE)
levels(df$age.quart) <- c("Young Adult", "Adult", "Middle Age", "Older")
age.vars <- c("Young Adult", "Adult", "Middle Age", "Older")
...
# Plot basic age ranges
  ...
  ggplot(df, aes(age.r)) + geom_histogram() + stat_ecdf(aes(y=..y..*6000)) + 
  scale_y_continuous(sec.axis=sec_axis(trans = ~./6000 , name="percentage", labels=percent)) +
    scale_x_continuous(minor_breaks = seq(0, 110, 5), breaks = seq(0, 100, 10)) + geom_vline(xintercept = 73)
...
sum(na.omit(df$age.r) > 73) / length(na.omit(df$age.r))
...
#Main results loop split by AGE QUARTILES
...
#for (v in 1:1) {
  ...
  # Note: For SPEED of plotting cut out the model if required
  
# model.quad <- lmer(outcome ~ gender +  subj_math_skills + gni + poly(age,2) + (1 | country) + (0 + subj_math_skills | country) + (0 + age | country), data = df.mod, control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
# 
...
# print(results.quad)
# 
# model.lin <- lmer(outcome ~ gender +  subj_math_skills + gni + age + (1 | country) + (0 + subj_math_skills | country) + (0 + age | country), data = df.mod, control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
# 
...
# print(results.lin)
  ...
  for (i in 1:4) {
    ...
    assign(paste0('plot.age',i), ggplot(df.mod[df.mod$age.quart == age.vars[i], ], aes(x = age.r, y = outcome)) + 
    ...
    scale_x_continuous(name = "Age", labels = label_number(accuracy = 1)) +
...
print(plot_grid(plot.age1, plot.age2, plot.age3, plot.age4, nrow = 1) + 
        draw_figure_label(label = var.lab, size = 15))
...
m1 <- lm(outcome ~ poly(age.r,2), data = df.mod)
ci <- data.frame(age = 15:99, predict(m1, data.frame(age.r = 15:99), interval = 'confidence'))
ci %>% ggplot(aes( x= age)) + geom_point(aes(y = upr)) + geom_point(aes(y=lwr))
ci %>% ggplot(aes( x= age)) + geom_point(aes (y = (upr-fit) ) ) + geom_point(aes (y = (lwr-fit) )) + geom_vline(xintercept = 83)

#Confidence interval at age 83
ci[69,"upr"]-ci[69,"lwr"]

#Geographical make up of different samples

total_obs <- sum(df %>% group_by(country) %>% count() %>% pull(n))
prop_all <- df %>% group_by(country) %>% count() %>% mutate(prop_all = 100 * n / total_obs) %>% select(-n)

total_obs <- sum(df %>% group_by(country) %>% filter(age.r <= 73) %>% count() %>% pull(n))
under_73 <- df %>% group_by(country) %>% filter(age.r <= 73) %>% count() %>% mutate(under_73 = 100 * n / total_obs) %>% select(-n)

total_obs <- sum(df %>% group_by(country) %>% filter(age.r <= 83) %>% count() %>% pull(n))
under_83 <- df %>% group_by(country) %>% filter(age.r <= 83) %>% count() %>% mutate(under_83 = 100 * n / total_obs) %>% select(-n)

prop <- left_join(prop_all, under_73, by='country') %>%
                left_join(., under_83, by='country') 

prop <- pivot_longer(prop, c(prop_all, under_73, under_83))

ggplot(prop, aes(fill=name, y=value, x=country)) +  geom_bar(position="dodge", stat="identity") + 
         scale_fill_brewer(palette = "Dark2") +
  coord_flip() + ylab("Proportion of sample") + theme(legend.position = "bottom", legend.title = element_blank(), axis.text.y = element_text(size = 5))


# Look at proportion of sample make up between 73 and 83

total_obs <- sum(df %>% group_by(country) %>% filter(age.r >= 73 & age.r <= 83) %>% count() %>% pull(n))
slice <- df %>% group_by(country) %>% filter(age.r >= 73 & age.r <= 83) %>% count() %>% mutate(p = 100 * n / total_obs) %>% select(-n)

ggplot (slice, aes(y=p, x=reorder(country, p))) +  geom_bar(stat="identity", fill="skyblue", alpha=0.7) + theme_bw() + coord_flip() + ylab("Proportion of sample") + theme(legend.position = "none", axis.text.y = element_text(size = 5))
...
print(results.lin)
  ...
  # Plot quadratic and linear fit
    ...
    geom_smooth(data = df.mod_83, color = 'black', fill = 'black', linetype = "dotted", method = "lm", formula = y ~ poly(x,2), alpha = 0.3, size = 0.75, show.legend = FALSE) + # quadratic to 83
      
    geom_smooth(data = df.mod_83, color = 'black', linetype = "dashed", method = "lm", formula = y ~ x, alpha = 0.3, size = 0.6, show.legend = FALSE, se = FALSE) +  #linear to 83
...
# Repeat process at different age cutoff (73)
```{r}
for (v in 1:length(vars)) {
# for (v in 1:1) {
  ...
  # create second dataframe cutting our all people over 73 years (95th percentile)
  df.mod_new <- df.mod
  df.mod_new$age.r[df.mod_new$age.r > 73] <- NA
  df.mod_new$age[df.mod_new$age.r > 73] <- NA
...
#    geom_smooth(color = dotcols[1], linetype = "solid", method = "lm", formula = y ~ x, alpha = 0.3, size = #0.6, show.legend = FALSE, se = FALSE) + #linear all ages
    ...
    geom_smooth(data = df.mod_new, color = 'black', fill = 'black', linetype = "dotted", method = "lm", formula = y ~ poly(x,2), alpha = 0.3, size = 0.75, show.legend = FALSE) + # quadratic to 73
...
df %>% group_by(age.r, continent) %>% filter(age.r <= age_cutoff) %>% summarise(across(prosocial_contrib, mean, na.rm= TRUE)) %>%  ggplot(aes(age.r, prosocial_contrib)) + geom_ma(aes(color = continent, group = continent), ma_fun = SMA, n=5, linetype = "solid", size = 1.5) + theme_bw()

#Loess smoothing
plot.contrib <- df %>% group_by(age.r, continent) %>% filter(age.r <= age_cutoff) %>% summarise(across(prosocial_contrib, mean, na.rm= TRUE)) %>%  ggplot(aes(age.r, prosocial_contrib)) + geom_smooth(aes(color = continent, group = continent, fill = continent), linetype = "solid", size = 1.5) + theme_bw()

#Plot Prosociality using COMPONENT LOADING from PCA rather than percentage CONTRIBUTION
plot.load <- df %>% group_by(age.r, continent) %>% filter(age.r <= age_cutoff) %>% summarise(across(prosocial_load, mean, na.rm= TRUE)) %>%  ggplot(aes(age.r, prosocial_load)) + geom_smooth(aes(color = continent, group = continent, fill = continent), linetype = "solid", size = 1.5) + theme_bw()

plot_grid(plot.contrib, plot.load)

#plot age vs prosociality WEIGHTED
df %>% group_by(age.r, continent) %>% filter(age.r <= age_cutoff) %>% summarise(across(prosocial_wgt_c, mean, na.rm= TRUE)) %>%  ggplot(aes(age.r, prosocial_wgt_c)) + geom_ma(aes(color = continent, group = continent), ma_fun = SMA, n=5, linetype = "solid", size = 1.5) + theme_bw()
