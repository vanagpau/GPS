...
vars <- c("posrecip", "negrecip", "altruism", "trust", "prosocial")
...
#create weighted prosociliaty measure
df$prosocial_wgt <- df$prosocial * df$wgt
...
dat.var$contrib        # Contributions to the PCs

# Create new prosociality composite based on these weightings
df$prosocial <- scale(dat.var$contrib[1]*df$posrecip + 
                     dat.var$contrib[2]*df$altruism + 
                     dat.var$contrib[3]*df$trust)
...
countrycols <- c("code", "region", "posrecip", "negrecip", "altruism", "trust", "prosocial", "n")
...
for (c in country.results$code) {
  ...
  cdf[,c("age", "posrecip", "negrecip", "altruism", "trust", "prosocial")] <- scale(cdf[,c("age", "posrecip", "negrecip", "altruism", "trust", "prosocial")])
  ...
  cntry.pr <- model_parameters(lm(prosocial ~ gender + age, data = cdf), standardize = "refit")
...
df %>% group_by(continent) %>% ggplot(aes(age.r, prosocial)) + geom_point()

# Create plots
```{r fig.align="center", fig.width=10, fig.height=8}

#plot age vs prosociality
df %>% group_by(age.r, continent) %>% filter(age.r <= 70) %>% summarise(across(prosocial, mean, na.rm= TRUE)) %>%  ggplot(aes(age.r, prosocial)) + geom_ma(aes(color = continent, group = continent), ma_fun = SMA, n=5, linetype = "solid", size = 1.5) + theme_bw()

#plot age vs prosociality WEIGHTED
df %>% group_by(age.r, continent) %>% filter(age.r <= 70) %>% summarise(across(prosocial_wgt, mean, na.rm= TRUE)) %>%  ggplot(aes(age.r, prosocial_wgt)) + geom_ma(aes(color = continent, group = continent), ma_fun = SMA, n=5, linetype = "solid", size = 1.5) + theme_bw()
...
world.prosocial <- ggplot(df.world, aes(long, lat, group = group)) +
  geom_polygon(aes(fill = prosocial), color = "white", size = .1) +
    scale_fill_gradient2(low = neg.corr.col, mid = "white", high = pos.corr.col, midpoint = 0, na.value = "#ECECEC", name = expression(beta)) +
  theme_void() +
